...
class TelegramBot:
    ...
    async def stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if category_stats:
            ...
            if 'whatsapp' in category_stats:
                stats_text += "\n**واتساب:**\n"
                stats_text += f"• مجموعات واتساب: {category_stats['whatsapp'].get('whatsapp_groups', 0):,}\n"
    ...
    async def categories_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        whatsapp_category = '📱 مجموعات واتساب'
        ...
        # زر واتساب
        if 'whatsapp' in category_stats:
            whatsapp_count = category_stats['whatsapp'].get('whatsapp_groups', 0)
            categories_text += f"\n**واتساب:**\n• {whatsapp_category}: {whatsapp_count:,}\n"
            keyboard_buttons.append([
                InlineKeyboardButton(
                    f"{whatsapp_category} ({whatsapp_count:,})", 
                    callback_data="export_whatsapp_all"
                )
            ])
    ...
    async def _handle_export_whatsapp_callback(self, query):
        ...
        try:
            db = await EnhancedDatabaseManager.get_instance()
            links = await db.export_links_by_category(platform='whatsapp', limit=Config.MAX_EXPORT_LINKS)
...
async def main():
    ...
    try:
        ...
        try:
            ...
            logger.info("📋 الأوامر: /start, /collect, /stop_collect, /export, /export_new, /categories, /stats")
